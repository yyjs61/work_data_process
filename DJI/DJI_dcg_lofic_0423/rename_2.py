...
def replace_dots_in_scenes(root_path):
    """
    功能：仅将场景文件夹名中的 '.' 替换为 'p'
    """
    if not os.path.exists(root_path):
    ...
    for scene in scenes:
        ...
        # 直接使用 Python 内置方法替换 '.' 为 'p'
        new_scene_name = scene.replace('.', 'p')
        
        # 如果名称没有变化（即原名称不含 '.'），跳过
        if scene == new_scene_name:
            print(f"  [跳过] {scene} (名称中不含 '.')")
            continue
        ...
        # 检查目标是否已存在，防止覆盖
        if os.path.exists(new_path):
            print(f"  [冲突] {scene} -> {new_scene_name} (目标文件夹已存在)")
            continue

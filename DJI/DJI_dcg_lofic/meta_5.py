...
# -*- coding: utf-8 -*-
"""
根据 received/<scene>/meta.txt 或 received/<scene>.txt 生成 yamls_eachFrame/<scene>/*.yaml
...
# 曝光时间配置（单位：ns）
EXPOTIME_NS = 33000000  # 10ms（根据实际调整）
...
def extract_scene_name(folder_name: str) -> str:
    """
    从 unpack_raw 的文件夹名中提取场景名（去掉数字前缀）
    例如：'00__20251209_203817-10p00ms-...' -> '20251209_203817-10.00ms-...'
    """
    # 去掉开头的数字和下划线前缀（如 00_, 01_, 02_ 等）
    match = re.match(r'^\d+__(.+)$', folder_name)
    if match:
        scene_name = match.group(1)
        # 将 p 替换回 . （如果文件名中有这种转换）
        scene_name = scene_name.replace('p', '.')
        return scene_name
    return folder_name


def find_meta_file(scene_name: str) -> str:
    """
    在 received 目录下查找对应的 meta.txt 文件
    支持两种格式：
    1. received/<scene_name>/meta.txt
    2. received/<scene_name>.txt
    """
    # 尝试1：received/<scene_name>/meta.txt
    meta_folder = os.path.join(RECEIVED_ROOT, scene_name)
    meta_file_in_folder = os.path.join(meta_folder, 'meta.txt')
    if os.path.exists(meta_file_in_folder):
        return meta_file_in_folder
    
    # 尝试2：received/<scene_name>.txt
    meta_txt_file = os.path.join(RECEIVED_ROOT, scene_name + '.txt')
    if os.path.exists(meta_txt_file):
        return meta_txt_file
    
    # 尝试3：received 目录下以 scene_name 开头的 txt 文件（模糊匹配）
    if os.path.exists(RECEIVED_ROOT):
        for file in os.listdir(RECEIVED_ROOT):
            if file.endswith('.txt') and scene_name.split('.')[0] in file:
                return os.path.join(RECEIVED_ROOT, file)
...
def main():
    ...
    for scene in scenes:
        ...
        # 从 unpack_raw 的文件夹名提取场景名（去掉数字前缀）
        scene_name_from_unpack = extract_scene_name(scene)
        print(f"🔍 提取场景名: {scene} -> {scene_name_from_unpack}")
        
        # 在 received 目录下查找对应的 meta.txt
        meta_file = find_meta_file(scene_name_from_unpack)
        
        if not meta_file or not os.path.exists(meta_file):
            print(f"⚠️  {scene}: 缺少 meta.txt (尝试查找: {scene_name_from_unpack})，跳过\n")
            continue
        
        scene_count += 1
        print(f"📄 找到 meta.txt: {os.path.basename(meta_file)}")

...
def write_yaml_file(filepath, data):
    ...
    with open(filepath, 'w') as f:
        ...
        f.write(f"SensorAGain: {data['SensorAGain']}\n")
        f.write(f"SensorDGain: {data['SensorDGain']}\n")
        f.write(f"sensorgain: {data['sensorgain']}\n")
        f.write(f"gain: {data['gain']}\n")
        f.write(f"isp_gain: {data['isp_gain']}\n")
        f.write(f"drc_gain: {data['drc_gain']}\n")
...
def main():
    ...
    for unpack_scene in unpack_scenes:
        ...
        print(f"处理场景: {unpack_scene} -> {received_scene_path}")
        ...
        for idx, raw_file in enumerate(raw_files):
            ...
            # 从txt获取SensorAGain和SensorDGain（aGain和dGain）
            # 注意：txt中字段名可能为 'aGain' 和 'dGain'，也可能为其他名称
            a_gain = meta_data.get('aGain', 1.0)
            d_gain = meta_data.get('dGain', 0.0)
            ...
            yaml_data = {
                ...
                # 'SensorAGain': float(a_gain),
                # 'SensorDGain': float(d_gain),
                'SensorAGain': meta_data.get('aGain', 1.0),
                'SensorDGain': float(1.0),
                ...
                # 'r_gain': float(r_gain),
                # 'b_gain': float(b_gain),
                'r_gain': meta_data.get('awbRGain'),
                'b_gain': meta_data.get('awbBGain'),

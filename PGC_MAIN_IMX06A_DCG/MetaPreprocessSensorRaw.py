...
def parseLine2(meta):
    with open(meta, 'r') as fo:
        ...
        # SensorAGain = min((float(line.split('sensitivity: ')[1].split(' ')[0]) / 100 / 16.0), 4.0)
...
for j, scene in enumerate(scenes):
    ...
    for i, file in enumerate(files):
        ...
        meta = os.path.join(os.path.join(RECEIVED, scene.split('__')[1], \
                                'awb_output_cam[0]_req_[' + file.split('req[')[1].split(']')[0] + '].txt'  ))    
        meta2 = os.path.join(os.path.join(RECEIVED, scene.split('__')[1], \
                                'sensor_info_cam[0]_req_[' + file.split('req[')[1].split(']')[0] + '].txt'  ))    
        


        # print(glob.glob(os.path.join(RECEIVED, scene.split('__')[1], 'Session*',\
        #                         'awb_output_*_req_*.txt'  )))
        
        # meta = glob.glob(os.path.join(RECEIVED, scene.split('__')[1],\
        #                         'awb_output_*_req_*' + file.split('req[')[1].split(']')[0] + '*.txt'  ))[0]    
        # meta2 = glob.glob(os.path.join(RECEIVED, scene.split('__')[1],\
